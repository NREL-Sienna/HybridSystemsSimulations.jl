...
bus_name = "chuhsi"
dic["λ_da_df"] =
    CSV.read("scripts/bilevel_pipeline/inputs/$(bus_name)_DA_prices.csv", DataFrame)
dic["λ_rt_df"] =
    CSV.read("scripts/bilevel_pipeline/inputs/$(bus_name)_RT_prices.csv", DataFrame)
dic["λ_Reg_Up"] =
    CSV.read("scripts/bilevel_pipeline/inputs/$(bus_name)_RegUp_prices.csv", DataFrame)
dic["λ_Reg_Down"] =
    CSV.read("scripts/bilevel_pipeline/inputs/$(bus_name)_RegDown_prices.csv", DataFrame)
dic["λ_Spin_Up_R3"] =
    CSV.read("scripts/bilevel_pipeline/inputs/$(bus_name)_Spin_prices.csv", DataFrame)
...
result_merch_DA = get_decision_problem_results(results, "MerchantHybridBilevel_DA")
result_merch_RT = get_decision_problem_results(results, "MerchantHybridBilevel_RT")
