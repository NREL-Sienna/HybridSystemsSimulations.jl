...
) where {T <: Float64, S <: PSY.Reserve}
    current_time = PSI.get_current_time(model)
    meta = PSI.get_attribute_key(attributes).meta
    if !(contains(meta, "R2") || contains(meta, "R1"))
...
function PSI._fix_parameter_value!(
    ...
    parameter_attributes::PSI.VariableValueAttributes{PSI.AuxVarKey{TotalBidReserve, S}},
) where {S <: PSY.Reserve}
