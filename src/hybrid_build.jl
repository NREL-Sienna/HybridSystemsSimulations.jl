...
function add_variable!(
    ...
    lb,
    ub::Float64,
) where {T <: PSI.VariableType, U <: Union{HybridOptimizer, HybridCoOptimizer}}
...
function PSI.build_impl!(decision_model::DecisionModel{HybridCoOptimizer})
    ...
    Δt_RT = 5 / 60
    Cycles = 4.11
    R = 0.333 # Ancillary Services Expected Deployment
    ...
    sb_spin_da_out = PSI.get_variable(container, regUpDABidOut(), HybridSystem)
    ...
    for t in T_da
        ...
        lin_cost_da_in = -Δt_DA * λ_da[t] * eb_da_in[t]
        lin_cost_on_th = -Δt_DA * C_th_fix * on_th[t]
        lin_cost_reg_up = Δt_DA * λ_da_regup[t] * sb_ru_da_out[t]
        lin_cost_spin = Δt_DA * λ_da_spin[t] * sb_spin_da_out[t]
        lin_cost_reg_down = Δt_DA * λ_da_regdown[t] * sb_rd_da_in[t]
        PSI.add_to_objective_invariant_expression!(container, lin_cost_da_out)
        PSI.add_to_objective_invariant_expression!(container, lin_cost_da_in)
        PSI.add_to_objective_invariant_expression!(container, lin_cost_on_th)
